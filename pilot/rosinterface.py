...
class PilotNode(object):
  ...
  def process_input(self, im):
    ...
    outputs, losses, aux_results = self.model.forward([im])
    
    left=sum(sum(outputs[2][0]))
    right=sum(sum(outputs[0][0]))
    straight=sum(sum(outputs[1][0]))
    # left=sum(sum(outputs[0][0]))
    # right=sum(sum(outputs[2][0]))
    # straight=sum(sum(outputs[1][0]))
    max_i=np.argmax([right,straight,left])
    action=max_i-1
    ...
    if FLAGS.show_depth and not self.finished:
      self.depth_pub.publish(outputs[max_i][0].flatten())
