    ...
    with tf.device(self.device):
      ...
      if self.FLAGS.network=='depth_q_net':
        ...
        if self.FLAGS.loss == 'ce':
          ...
          self.loss = -tf.multiply(self.targets, tf.log(tf.clip_by_value(self.predictions_train,0.0001,1)))-tf.multiply((1-self.targets),tf.log(tf.clip_by_value(1-self.predictions_train,0.00001,1)))
