...
def depth_q_net(inputs,
  ...
  Args:
    inputs: a tensor of shape [batch_size, height, width, channels].
    actions: a tensor of shape [action_dim].
    ...
    scope: Optional variable_scope.
  Returns:
    depth predictions: a list of tensors of shape [batch_size, 55, 74]
  ...
  with tf.variable_scope(scope, 'DepthQnet', reuse=reuse) as scope:
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      with tf.variable_scope('q_depth'):
        for output in range(FLAGS.num_outputs):
          end_point = '{}_depth_fc_0'.format(output)
          depth_feat = tf.reshape(net,[-1, int(FLAGS.depth_multiplier*1024)])
          ...
          end_point = '{}_depth_fc_1'.format(output)
          depth_feat=slim.fully_connected(depth_feat, depth_size[0]*depth_size[1], tf.nn.relu)
          end_points[end_point] = depth_feat

          # output height 55 width 74
          end_point = '{}_depth_reshaped'.format(output)
          predictions=tf.reshape(depth_feat, [-1, depth_size[0],depth_size[1]])
          end_points[end_point] = predictions
          depth_predictions.append(predictions)
  return depth_predictions, end_points

depth_q_net.default_image_size={}
depth_q_net.default_image_size[1] = 224
depth_q_net.default_image_size[0.5] = 160
depth_q_net.default_image_size[0.25] = 128
...
def depth_q_net_arg_scope(is_training=True,
