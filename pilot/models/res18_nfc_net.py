...
class Net(nn.Module):
  ...
  def __init__(self, output_size=10, pretrained=False, n_frames=5, **kwargs):
    ...
    feature_network = res18.Net(pretrained=pretrained)
    self.H=512
    self.n_frames=n_frames
    self.default_feature_size=self.n_frames*np.prod(feature_network.default_feature_size)
    self.default_image_size=feature_network.default_image_size
    self.cnn = feature_network.network.feature
  ...
  def forward(self, inputs, train=False, verbose=False):
    """
