...
def tinynet(inputs,
    ...
    ep=tf.layers.dense(tf.reshape(ep,[-1,20*20*20]), 1024, activation=tf.nn.relu, use_bias=True, kernel_initializer=tf.contrib.layers.xavier_initializer(), name=end_point, reuse=reuse)
    end_points[end_point]=ep                    
