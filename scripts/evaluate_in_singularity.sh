...
for model in  all_factors/ensemble_mse_1/0 ; do
  ...
  python run_script.py -t ${model}_eva_opal -pe sing -pp ensemble_v0/pilot -m $model -w corridor --reuse_default_world -p eva_params.yaml -n 10 --robot drone_sim --fsm oracle_nn_drone_fsm -e
  # done
done
