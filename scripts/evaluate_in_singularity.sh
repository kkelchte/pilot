...
for model in  all_factors/tiny_ens0_new1_max all_factors/tiny_ens0_new1_weighted_mean  ; do
  ...
  python run_script.py -t ${model}_eva -pe sing -pp ensemble_v0/pilot -m $model -w corridor --reuse_default_world -p eva_params.yaml -n 10 --robot drone_sim --fsm oracle_nn_drone_fsm -e
  # done
done
