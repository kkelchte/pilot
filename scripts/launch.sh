...
for i in $(seq 0 9) ;  do
  python dag_train_and_evaluate.py -t dynamic_ensemble_10_frozen/image/mobile_imgnet/$i --wall_time_train $((7*60*60)) --wall_time_eva $((2*60*60)) --checkpoint_path static_ensemble_10/mobile_imgnet/$i --number_of_models 1 --loss mse --discriminator_input image --non_expert_weight 0 --load_data_in_ram --learning_rate 0.05 --dataset all_factors --discrete --max_episodes 1000 --visualize_deep_dream_of_output --visualize_saliency_of_output --paramfile eva_params.yaml --number_of_runs 10 -w corridor -w esatv1 --reuse_default_world --robot drone_sim --fsm oracle_nn_drone_fsm --evaluation --speed 1.3	
done
