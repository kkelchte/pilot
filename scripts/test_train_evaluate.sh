...
python ~/tensorflow/pytorch_pilot/pilot/main.py --max_episodes 3 --dataset canyon_drone_tiny --network mobile --log_tag test_model --owr --discrete --scratch #--load_data_in_ram 
ret=$?
if [ $ret -ne 0 ]; then
     #Handle failure
     exit
...
singularity exec --nv /esat/opal/kkelchte/singularity_images/ros_gazebo_tensorflow_writable.img /esat/opal/kkelchte/docker_home/tensorflow/pytorch_pytorch_pilot/scripts/evaluate_in_singularity.sh
